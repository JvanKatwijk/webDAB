...
void HttpListener::listen () {
	...
	quint16 port	= settings -> value ("port", "8080").toUInt() & 0xFFFF;
	settings	-> endGroup ();
	QTcpServer::listen (host.isEmpty() ?
	                    QHostAddress::Any : QHostAddress (host), port);
...
//	Let the handler process the new connection.
	if (freeHandler) {
//	The descriptor is passed via event queue because the
//	handler lives in another thread
	   QMetaObject::invokeMethod (freeHandler,
	                              "handleConnection",
	                               Qt::QueuedConnection,
	                               Q_ARG(tSocketDescriptor,
	                               socketDescriptor));
	}
	else {
