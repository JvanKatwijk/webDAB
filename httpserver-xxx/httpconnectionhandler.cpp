...
using namespace stefanfrings;
	...
	HttpConnectionHandler::
	         HttpConnectionHandler (const QSettings *settings,
...
//	Connect signals
	...
	qDebug ("HttpConnectionHandler (%p): constructed",
	                                     static_cast<void*>(this));    
}
...
void	HttpConnectionHandler::thread_done () {
	...
	thread -> wait ();
	thread -> deleteLater ();
	qDebug ("HttpConnectionHandler (%p): destroyed",
	                                    static_cast<void*>(this));
}

void	HttpConnectionHandler::createSocket() {
//	If SSL is supported and configured,
//	then create an instance of QSslSocket
#ifdef QT_NO_SSL
...
void	HttpConnectionHandler::
	               handleConnection (tSocketDescriptor socketDescriptor) {
	qDebug ("HttpConnectionHandler (%p): handle new connection",
	                                           static_cast<void*>(this));
	busy = true;
// if open already, then the handler is already busy
	Q_ASSERT (socket -> isOpen() == false);

//	UGLY workaround - we need to clear writebuffer before
//	reusing this socket
...
void	HttpConnectionHandler::disconnected () {
	qDebug ("HttpConnectionHandler (%p): disconnected",
	                                       static_cast<void*>(this));
	socket -> close();
	readTimer. stop();
	busy = false;
...
//	If the request is aborted, return error message
...
//	Find out whether the connection must be closed
	      if (!closeConnection) {
//	Maybe the request handler or mapper added a
//	Connection:close header in the meantime
	         bool closeResponse =
...
//	If we have no Content-Length header and did not
//	use chunked mode, then we have to close the
//	connection to tell the HTTP client that the end of the
//	response has been reached.
	            bool hasContentLength =
	                    response. getHeaders().contains("Content-Length");
	            if (!hasContentLength) {
	               bool hasChunkedMode =
	                    QString::compare(response.getHeaders().value("Transfer-Encoding"),"chunked",Qt::CaseInsensitive)==0;
	               if (!hasChunkedMode) {
	                  closeConnection = true;
	               }
	            }
...
//	Close the connection or prepare for the next request on
//	the same connection.
	      if (closeConnection) {
	         while (socket -> bytesToWrite ())
	            socket -> waitForBytesWritten ();
