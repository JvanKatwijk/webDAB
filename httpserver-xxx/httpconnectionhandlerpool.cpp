...
using namespace stefanfrings;
	...
	HttpConnectionHandlerPool::
	           HttpConnectionHandlerPool (QSettings *settings,
...
// create a new handler, if necessary
	if (!freeHandler) {
	   int maxConnectionHandlers =
	                settings -> value ("maxThreads",100).toInt();
	   if (pool. count() < maxConnectionHandlers) {
	      freeHandler =
	              new HttpConnectionHandler (settings,
	                                         requestHandler,
	                                         sslConfiguration);
	      freeHandler -> setBusy();
...
//	based on the directory of the config file.
	   ...
	   if (QDir::isRelativePath(sslKeyFileName)) {
#endif
	      sslKeyFileName = QFileInfo (configFile. absolutePath (),
	                                  sslKeyFileName).absoluteFilePath();
	   }
#ifdef Q_OS_WIN32
	   ...
	   if (QDir::isRelativePath(sslCertFileName)) {
	      ...
	      sslCertFileName = 
	               QFileInfo (configFile.absolutePath (),
	                          sslCertFileName). absoluteFilePath ();
            }
