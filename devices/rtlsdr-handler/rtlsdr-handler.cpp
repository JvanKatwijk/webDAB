...
int32_t	r;
	...
	Handle		= dlopen ("libusb-1.0", RTLD_NOW);
...
//	Ok, from here we have the library functions accessible
	...
	fprintf (stderr, "we found %d dabsticks\n", deviceCount);
//
	...
	int gain	= theGain * gainsCount / 100;
...
void	rtlsdrHandler::setGain	(int gain) {
	theGain	= gain;
}

bool	rtlsdrHandler::has_agc	() {
	return true;
}

void	rtlsdrHandler::set_agc	(bool b) {
	rtlsdr_set_agc_mode (device, b ? 1 : 0);
}
...
//
	...
	rtlsdr_set_tuner_bandwidth      = (pfnrtlsdr_set_tuner_bandwidth)
