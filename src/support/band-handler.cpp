...
struct dabFrequencies *finger = bandIII_frequencies;

	for (int i = 0; finger[i]. key != nullptr; i++) {
	   if (QString (finger[i]. key) == Channel) {
	      return finger [i]. fKHz * 1000;
	      break;
	   }
	}

	return finger [0].fKHz * 1000;
}

int	bandHandler::channelNr	(const QString &Channel) {
struct dabFrequencies *finger = bandIII_frequencies;
