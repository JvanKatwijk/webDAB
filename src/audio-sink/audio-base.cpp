...
int32_t	result;
	...
	for (i = 0; i < amount; i ++)
	   if (converter_16. convert (std::complex<float> (V [2 * i] / 32767.0,
	                              ...
	                              outputBuffer, &result)) {
	      for (j = 0; j < result; j ++) {
	         buffer [2 * j    ] = theGain * real (outputBuffer [j]);
	         buffer [2 * j + 1] = theGain * imag (outputBuffer [j]);
	      }
...
int32_t	i;
	...
	for (i = 0; i < amount; i ++) {
	   buffer [2 * i]	= theGain * V [2 * i] / 32767.0;
	   buffer [2 * i + 1]	= theGain * V [2 * i + 1] / 32767.0;
	}
...
void	audioBase::setGain	(int gain) {
	theGain = gain / 100.0;
}
