...
void HttpRequest::readRequest(QTcpSocket* socket) {
	...
	#ifdef SUPERVERBOSE
	   fprintf (stderr, "HttpRequest: read request\n");
	#endif
// allow one byte more to be able to detect overflow
	...
	if (!lineBuffer.contains("\r\n")) {
	   #ifdef SUPERVERBOSE
	      fprintf (stderr, "HttpRequest: collecting more parts until line break\n");
	   #endif
	   return;
	}
	...
	lineBuffer. clear();
	if (!newData. isEmpty()) {
//	   fprintf (stderr, "HttpRequest: from %s: %s\n",
...
void HttpRequest::readHeader(QTcpSocket* socket) {
	...
	if (colon>0) {
        ...
        #ifdef SUPERVERBOSE
            fprintf(stderr, "HttpRequest: received header %s: %s\n",
        ...
        #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: read additional line of header\n");
        #endif
    ...
    else
    {
        ...
        #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: headers completed\n");
        #endif
        ...
        {
            #ifdef SUPERVERBOSE
                fprintf (stderr, "HttpRequest: expect no body\n");
            #endif
            status=complete;
        }
        ...
        else if (!boundary.isEmpty() && expectedBodySize>maxMultiPartSize)
        {
            ...
            #ifdef SUPERVERBOSE
                fprintf (stderr, "HttpRequest: expect %i bytes body\n",expectedBodySize);
            #endif
            status = waitForBody;
        }
    }
...
// normal body, no multipart
	   #ifdef SUPERVERBOSE
	      fprintf (stderr, "HttpRequest: receive body\n");
	   #endif
	   int toRead = expectedBodySize - bodyData.size();
	   QByteArray newData = socket->read(toRead);
//	   fprintf (stderr, "reading body %s\n",
//	                    QString (newData). toLatin1 (). data ());
...
//	multipart body, store into temp file
	   #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: receiving multipart body\n");
        #endif
        ...
        {
        #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: received whole multipart body\n");
        #endif
            tempFile->flush();
            if (tempFile->error())
            {
...
{
    #ifdef SUPERVERBOSE
        fprintf (stderr, "HttpRequest: extract and decode request parameters\n");
    #endif
    // Get URL parameters
...
{
    #ifdef SUPERVERBOSE
        fprintf (stderr, "HttpRequest: extract cookies\n");
    #endif
    foreach(QByteArray cookieStr, headers.values("cookie"))
    {
        ...
        {
            #ifdef SUPERVERBOSE
                fprintf (stderr, "HttpRequest: found cookie %s\n",part.data());
...
//	fprintf (stderr, "HttpRequest: parsing multipart temp file\n");
    tempFile->seek(0);
    bool finished=false;
    while (!tempFile->atEnd() && !finished && !tempFile->error())
    {
        #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: reading multpart headers\n");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("Content-Disposition:"))
            {
                if (line.contains("form-data"))
                {
                    ...
                    #ifdef SUPERVERBOSE
                        fprintf (stderr, "HttpRequest: multipart field=%s, filename=%s\n",fieldName.data(),fileName.data());
                    #endif
                }
                else
                {
//	        fprintf (stderr, "HttpRequest: ignoring unsupported content part %s\n",line.data());
                }
            }
        ...
        #ifdef SUPERVERBOSE
            fprintf (stderr, "HttpRequest: reading multpart data\n");
        #endif
        ...
        while (!tempFile->atEnd() && !finished && !tempFile->error())
        {
            ...
            if (line.startsWith("--"+boundary))
            {
                ...
                if (fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    fprintf (stderr, "HttpRequest: set parameter %s=%s\n",fieldName.data(),fieldValue.data());
                }
                else if (!fileName.isEmpty() && !fieldName.isEmpty())
                {
                    ...
                    {
                        #ifdef SUPERVERBOSE
                            fprintf (stderr, "HttpRequest: finishing writing to uploaded file\n");
                        #endif
                        ...
                        fprintf (stderr, "HttpRequest: uploaded file size is %lli\n",uploadedFile->size());
                    }
                    else
                    {
    ...
    #ifdef SUPERVERBOSE
        fprintf (stderr, "HttpRequest: finished parsing multipart temp file\n");
    #endif
}
